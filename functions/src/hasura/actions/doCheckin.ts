...
export default async (req: Request, res: Response) => {
  ...
  if (result.users_by_pk.checkins_aggregate.aggregate.count > 0) {
    ...
    return res.status(400).json({message: 'ERR_TODAY_CHECKED'});
  } else {
    console.log(`User ${userId} no checkin `);
    const point = process.env.POINT_CHECKIN || 1;
    const amount = +point * result.users_by_pk.rank.checkin_rate;
    try {
      const {insert_checkins_one} = await GraphqlAdmin.request<
        any,
        CheckinVars
      >(INSERT_CHECKIN, {
        user_id: userId,
      ...
      return res.json({amount, created_at: insert_checkins_one.created_at});
    } catch (error) {
      console.error(error);
      return res.status(400).json({message: 'ERR_CHECKIN'});
    }
  }
};
